...
import { Target, Timer, Zap, Sparkles } from 'lucide-react';
...
export default function StatsPanel({
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6" id="stats-panel-container">
      {/* Timer Display */}
      <div className="bg-surface border border-border-subtle rounded-2xl p-4 flex flex-col justify-between shadow-lg relative overflow-hidden">
        ...
        <div>
          ...
          <span className="text-xs text-muted font-mono ml-1">s</span>
        </div>
        <div className="absolute bottom-0 left-0 h-[2px] bg-accent transition-all duration-100" style={{ width: `${progress}%` }} />
      </div>

      {/* Accuracy Metric */}
      <div className="bg-surface border border-border-subtle rounded-2xl p-4 flex flex-col justify-between shadow-lg">
        <div className="flex items-center justify-between text-muted mb-2">
        ...
        <div>
          ...
          <span className="text-xs text-muted font-mono ml-1">%</span>
        </div>
      ...
      <div className="bg-surface border border-border-subtle rounded-2xl p-4 flex flex-col justify-between shadow-lg">
        <div className="flex items-center justify-between text-muted mb-2">
          ...
          <Zap className="w-4 h-4 text-muted animate-pulse" />
        </div>
        <div>
          <span className="text-2xl font-black text-primary font-mono leading-none">
            {cps.toFixed(2)}
          </span>
          <span className="text-xs text-muted font-mono ml-1">CPS</span>
        </div>
        <p className="text-[10px] text-muted font-mono mt-1">
      ...
      <div className="bg-surface border border-border-subtle rounded-2xl p-4 flex flex-col justify-between shadow-lg">
        ...
        <div>
          ...
          {bestTime && <span className="text-xs text-muted font-mono ml-1">s</span>}
        </div>
        <p className="text-[10px] text-muted font-mono mt-1">
